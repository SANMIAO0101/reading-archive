...
function pageHead(num,title,desc,actions=''){return `<header class="page-head"><div class="page-head-main"><div class="page-head-title"><div class="index">${num}</div><h2>${title}</h2></div><div class="page-head-side"><p class="page-desc">${desc}</p><div class="actions">${actions}</div></div></div></header>`}
...
function pageFooter(){
  const i=PAGE_ORDER.indexOf(route);
  const prev=i>0?PAGE_ORDER[i-1]:null;
  const next=i>=0&&i<PAGE_ORDER.length-1?PAGE_ORDER[i+1]:null;
  return `<footer class="page-footer"><div class="page-footer-line"></div><nav class="page-pager" aria-label="Page navigation"><button class="pager-btn" ${prev?`onclick="go('${prev}')"`:'disabled'}><span class="pager-k">PREV</span><span class="pager-v">${prev?PAGE_LABEL[prev]:'—'}</span></button><button class="pager-btn pager-home" onclick="go('home')"><span class="pager-k">HOME</span><span class="pager-v">ARCHIVE</span></button><button class="pager-btn" ${next?`onclick="go('${next}')"`:'disabled'}><span class="pager-k">NEXT</span><span class="pager-v">${next?PAGE_LABEL[next]:'—'}</span></button></nav></footer>`;
}
